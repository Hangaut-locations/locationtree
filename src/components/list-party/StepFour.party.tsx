...
const PartyStepFour: React.FC<IStepProps> = ({ photos, setPhotos }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-200 ease-out">
      {/* Header */}
      <div>
        <p className="text-sm font-semibold text-purple-950 dark:text-purple-300 uppercase tracking-widest block">
          Step 4
        </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {photos.map((photo, index) => (
              ...
              >
                ...
                <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-foreground">
                  Photo {index + 1}
                </span>
                ...
                <button
                  ...
                  className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/60 text-white opacity-0 backdrop-blur-sm transition-all group-hover:opacity-100 hover:bg-red-600"
