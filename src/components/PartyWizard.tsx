import { CalendarDays, HelpCircle, MapPin, Minus, PartyPopper, Plus, Ticket, Type, Users } from "lucide-react"
...
export const PartyWizard: React.FC<PartyWizardProps> = ({ onAddListing }) => {
  ...
  const handleExit = () => {
    if (window.confirm("Are you sure you want to save and exit?")) navigate("/")
  }
  ...
  const handleSubmit = () => {
    ...
    const finalTitle =
      title.trim() || `${partyType} in ${normalizedLocation}${priceMode === "person" ? ` — ${capacity} guests` : ""}`
  ...
  const isStepValid = () => {
    ...
    if (step === 3) return ["paris", "watford", "london"].includes(location.trim().toLowerCase())
    if (step === 5) return activities.trim().length > 0
    if (step === 8) return price > 0
    return true
  }

  const renderCounter = (label: string, value: number, onChange: (val: number) => void, min = 1) => {
    return (
                ...
                {PARTY_TYPES.map((type) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-2xl border px-4 py-3.5 text-left text-sm font-black transition-[border-color,background-color,transform] duration-160 ease-out cursor-pointer active:scale-97 ${
                        isSelected
                    ...
                    >
                      ...
                      <span className="text-xs font-black text-foreground">{type}</span>
                    </button>
                  )
                })}
              </div>
              ...
              <div className="max-w-md mx-auto space-y-4">
                <div className="flex items-center gap-3 border border-border/80 bg-card rounded-2xl px-4 py-3.5 transition-colors focus-within:border-purple-600">
                  ...
                  <label className="flex flex-col w-full">
                    <span className="text-[10px] font-black uppercase text-muted-foreground">Start date</span>
                    <input
                      type="date"
                      min={today}
                      value={startDate}
                ...
                <div className="flex items-center gap-3 border border-border/80 bg-card rounded-2xl px-4 py-3.5 transition-colors focus-within:border-purple-600">
                  <CalendarDays className="h-5 w-5 text-muted-foreground" />
                  <label className="flex flex-col w-full">
                    <span className="text-[10px] font-black uppercase text-muted-foreground">End date</span>
                    <input
                      type="date"
                      min={startDate || today}
                      value={endDate}
                ...
                {startDate && endDate && !isDateValid && (
                  <p className="text-center text-xs font-bold text-red-500">End date must be after the start date.</p>
                )}
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col gap-4 max-w-md mx-auto">
                <div className="flex items-center gap-3 border border-border/80 bg-card rounded-full px-5 py-3.5 transition-colors focus-within:border-purple-600 focus-within:ring-2 focus-within:ring-purple-600/10 w-full">
                  ...
                  <input
                    type="text"
                    placeholder="Party location (e.g. Watford, Paris, London)"
                    value={location}
          ...
          {step === 4 && (
            <div className="space-y-6">
              <div className="space-y-1.5 text-center">
                ...
                <p className="text-xs font-semibold text-muted-foreground">
                  Parties don't need bedrooms or beds — guests come for the experience, not to sleep.
                </p>
              </div>

              <div className="max-w-md mx-auto">{renderCounter("Guest capacity", capacity, setCapacity)}</div>

              <div className="space-y-3">
                <p className="text-sm font-black text-foreground">How do you want to charge?</p>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-black text-foreground">Per person</p>
                      <p className="text-[11px] font-semibold text-muted-foreground">Guests buy a ticket per seat</p>
                    </div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-black text-foreground">Per hour</p>
                      <p className="text-[11px] font-semibold text-muted-foreground">Guests book the whole party</p>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-3 max-w-md mx-auto">
                <span className="text-5xl font-black text-foreground">$</span>
                <input
                  type="number"
                  min={1}
                  value={price}
                  onChange={(e) => setPrice(Number(e.target.value) || 0)}
                  className="w-32 border-r border-border/60 bg-transparent text-center text-5xl font-black text-foreground outline-none"
                />
                <span className="text-sm font-black text-muted-foreground">/ {priceMode}</span>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div className="space-y-6">
              <div className="space-y-1.5 text-center">
                ...
                <p className="text-xs font-semibold text-muted-foreground">
                  Describe the vibe, music, activities and anything guests should expect.
                </p>
              </div>
              <textarea
                value={activities}
          ...
          {step === 6 && (
            <div className="space-y-6">
              <div className="space-y-1.5 text-center">
                ...
                <h2 className="text-xl sm:text-2xl font-black text-foreground tracking-tight">Set your party rules</h2>
                <p className="text-xs font-semibold text-muted-foreground">
          ...
          {step === 7 && (
            <div className="space-y-6">
              <div className="space-y-1.5 text-center">
                ...
                <h2 className="text-xl sm:text-2xl font-black text-foreground tracking-tight">Name your party</h2>
                <p className="text-xs font-semibold text-muted-foreground">
                  Pick something catchy — you can tweak it anytime from your host profile.
                </p>
              </div>
              <input
                type="text"
              ...
              />
              <p className="text-right text-xs font-semibold text-muted-foreground">{title.length}/48</p>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-black text-foreground">Guests book / buy tickets for reservation</p>
                  <p className="text-xs font-semibold text-muted-foreground">
                    Ticket sales are required for this party. Turn off to allow free RSVP.
                  </p>
                </div>
                <span
                  className={`h-6 w-6 rounded-full border flex items-center justify-center text-[10px] font-black ${
                    ticketed ? "bg-purple-950 text-white border-purple-950" : "border-border text-transparent"
                  }`}
                >
                  ✓
                </span>
          ...
          {step === 8 && (
            <div className="space-y-8 text-center">
              <div className="space-y-1.5">
                ...
                <p className="text-xs font-semibold text-muted-foreground">
                  <span className="font-black text-foreground">{partyType}</span> · {startDate} → {endDate} · {capacity}{" "}
                  guests
                </p>
              </div>
              ...
              <p className="text-sm font-black text-muted-foreground">per {priceMode}</p>
              ...
              <div className="max-w-md mx-auto rounded-3xl border border-border bg-card p-5 text-left space-y-2">
                ...
                <div className="flex justify-between text-xs font-bold text-muted-foreground">
                  ...
                  <span className="text-foreground">{ticketed ? "Tickets required" : "Free RSVP"}</span>
                </div>
              </div>
