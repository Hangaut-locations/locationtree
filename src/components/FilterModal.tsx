...
export const FilterModal: React.FC<FilterModalProps> = ({
  ...
  const [placeType, setPlaceType] = useState<FilterState["placeType"]>(
    filters.placeType,
  );
      ...
      <DialogContent className="w-full max-w-full sm:max-w-[680px] h-full sm:h-auto rounded-none sm:rounded-[32px] border-t sm:border border-border bg-card p-0 shadow-2xl animate-in fade-in sm:zoom-in-95 duration-200 top-0 left-0 sm:top-1/2 sm:left-1/2 translate-x-0 translate-y-0 sm:-translate-x-1/2 sm:-translate-y-1/2 overflow-hidden">
        ...
        <div className="px-5 py-6 sm:px-10 sm:py-8 space-y-8 overflow-y-auto max-h-[calc(100vh-130px)] sm:max-h-[70vh] scrollbar-none">
          ...
          <div className="rounded-full border border-gray-300 dark:border-border p-1.5 bg-white dark:bg-muted/10 flex items-center w-full">
            <button
              ...
              className={`flex-grow py-4 text-center text-base font-bold rounded-full transition-[background-color,color,transform] duration-160 ease-out active:scale-97 cursor-pointer ${
                placeType === "any"
            ...
            <button
              ...
              className={`flex-grow py-3 text-center text-sm font-bold rounded-[20px] transition-[background-color,color,transform] duration-160 ease-out active:scale-97 cursor-pointer ${
                placeType === "room"
                  ? "bg-purple-950 text-white shadow-sm dark:bg-purple-800"
                  : "text-purple-950 dark:text-purple-300 hover:bg-muted/30"
              }`}
          ...
          <div className="space-y-4">
            <label className="text-sm font-bold text-gray-800 dark:text-gray-200 tracking-tight block">
              Price range
            </label>
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex flex-col items-start gap-1.5">
                <span className="text-xs font-bold text-gray-500">Minimum</span>
                <div className="flex items-center justify-center border border-black dark:border-white rounded-xl px-4 py-1.5 bg-white dark:bg-muted/20">
                  ...
                  <input
                    ...
                    className="w-14 bg-transparent text-sm font-bold text-foreground outline-none border-none p-0 focus:ring-0 text-center"
                  />
                </div>
              </div>
              <div className="flex flex-col items-end gap-1.5">
                <span className="text-xs font-bold text-gray-500">Maximum</span>
                <div className="flex items-center justify-center border border-black dark:border-white rounded-xl px-4 py-1.5 bg-white dark:bg-muted/20">
                  <span className="text-sm font-bold text-foreground">$</span>
                  <input
                    type="number"
                    value={maxPrice}
                    onChange={(e) => setMaxPrice(Number(e.target.value))}
                    className="w-16 bg-transparent text-sm font-bold text-foreground outline-none border-none p-0 focus:ring-0 text-center"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between border-t border-border/60 px-5 sm:px-8 py-4 sm:py-5 bg-card mt-auto sm:rounded-b-[32px] rounded-none">
          <button
            ...
            className="text-sm font-bold text-foreground hover:text-purple-800 transition-colors cursor-pointer"
          >
            Clear all
          </button>
