...
function App() {
  ...
  const activeListingsSource =
    activeTab === "location"
      ? listingsList.filter(
          (l) => l.hostingType !== "party" && !planningIds.has(l.id),
        )
      : listingsList.filter(
          (l) => l.hostingType === "party" || planningIds.has(l.id),
        );
  ...
  const filteredListings = activeListingsSource.filter((listing) => {
    ...
    if (filters.placeType !== "any") {
      ...
      if (filters.placeType === "room") {
        const isRoom =
          titleLower.includes("room") || titleLower.includes("suite");
  ...
  const wizardRoutes = (
    <Routes>
      ...
      />
      <Route
        path="/become-a-host/party"
        element={<PartyWizard onAddListing={handleAddNewListing} />}
      />
      <Route
